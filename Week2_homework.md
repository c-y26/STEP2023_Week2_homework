...
- SampleCodeから、最も新しく追加された要素が、ハッシュテーブルへのアドレスを持っていることがわかる
- 最も新しい要素を削除する場合に限り、特別な処理が必要であることに注意して実装した

#### 2. ハッシュテーブルのサイズを素数に
- 素数の生成方法：エラトステネスのふるいを用いる
    - 計算量は、O(NloglogN)で、ほぼO(N)
    - 再ハッシュの際に、テーブルサイズを決定するのに用いる
- エラトステネスのふるい(N以下の素数を全て求める場合)
   1. 2からNまでの一覧表を作る
   2. 2に丸をつけて、2以外の2の倍数にバツをつける
   3. 次は3に丸をつけて、3以外の3の倍数にバツをつける
   4. このように、まだ丸がついていない最も小さい数に丸をつけて、その倍数は全て消すという作業を繰り返す。
   5. これをN**(1/2)まで繰り返す
#### 3. 再ハッシュの実装
- 要素数がテーブルサイズの70%を上回ったら、テーブルサイズを約2倍に拡張
- 要素数がテーブルサイズの30%を下回ったら、テーブルサイズを約1/2に縮小
- 「約」としているのは、テーブルサイズをこれに最も近い素数に調節するためである
- rehash_item(item): 与えられたitemのハッシュ値を再度計算し、新しいハッシュテーブルに格納する
...
functional_test()、performance_test()共に通過した。
...
1. ハッシュテーブルでは、データ数に対して十分大きいテーブルサイズを確保する必要がある。しかし、データ数がわからないという状況下で適切なテーブルサイズを決めるのが困難である。これに対して、木構造では要素を追加する際にメモリを確保すれば良く、事前にデータ数を知っておく必要がないというメリットがある。
2. データ数がどの程度になるか不明な状態で、ハッシュ関数を適切に決定するのは難しい。これによってハッシュの衝突が頻繁に起こってしまうと、最悪時間計算量がO(n)となってしまう。こうなると、バランス木などの方が効率が良い。
4. ハッシュテーブルの方が必要以上にメモリを確保しておく必要があるため、空間計算量が多くなってしまう。
5. ハッシュテーブルの場合、要素の順番がバラバラに格納される。それに対して、木構造では要素を整列された状態で取り出すことができる。
...
- class Item (hash_table.py)
    - key, value, next: デフォルト
    - time_item: 宿題4にあたって追加。TimeItemクラス(time_table.py)のインスタンス。

- class TimeItem (time_table.py) 
    - url: どのItemに対するtime_itemか
    - prev: 直前のtime_itemを指す 
    - next: 直後のtime_itemを指す  
    
- class DoubleLinkedList (time_table.py)
    - 各Itemクラスのtime_itemを双方向リストで連結
    - headとtailはダミーとして初期化
    - add_node(time_item): 与えられたtime_itemをheadの直後に追加
    - remove_node(time_item): 与えられたtime_itemを削除する
    - move_to_head(time_item): 与えられたtime_itemをheadの直後に追加
    - remove_tail_item: 最も古いノードを削除し、削除したノードのurlを返す

#### access_page()について
1. アクセスしたページがキャッシュに保存されているなら、その要素をitemに格納
2. itemがNoneでない場合、キャッシュの順序のみ変更すれば良いので、linked_listのみ変更する
3. itemがNoneの場合、hash_tableにItemを追加し、その要素time_itemをlinked_listで繋ぎかえる
4. 同時に、最も古い要素は削除する(キャッシュに保存できる上限を超えた場合

#### get_pages()について
キャッシュに保存されている要素を、新しいものから順に表示する。

linked_listでは、要素が時系列順に並んでいる。
そのため、これをheadからtailまでたどれば良い。
